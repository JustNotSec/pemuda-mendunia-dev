  ...
  <div class="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 py-20">
    <div class="relative mx-auto max-w-4xl grid space-y-5 sm:space-y-10">
      ...
      <div class="text-center">
          <h1 class="text-2xl text-gray-800 font-bold sm:text-5xl lg:text-4xl lg:leading-tight">
              Ingin Berangkat Edutrip Sekolah ke Luar Negeri Bersama<span class="text-red-600">Pemuda Mendunia?</span>
            </h1>
            <p class="text-xs font-bold italic bg-red-600 rounded-full text-white tracking-wide uppercase mb-3">
            Segera Konsultasikan Keberangkatan Edutrip Sekolahmu Sekarang!
            </p>
      </div>
      <!-- End Title -->
      ...
      <div class="sm:flex sm:justify-center sm:items-center text-center sm:text-start">
        <div class="shrink-0 pb-5 sm:flex sm:pb-0 sm:pe-5">
          ...
          <div class="flex justify-center -space-x-3">
            ...
            <img class="inline-block size-8 rounded-full ring-2 ring-white" src="https://images.unsplash.com/photo-1492562080023-ab3db95bfbce?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=facearea&facepad=2&w=300&h=300&q=80" alt="Avatar">
            <span class="inline-flex items-center justify-center size-8 rounded-full ring-2 ring-white bg-gray-800">
              <span class="text-xs font-medium leading-none text-white uppercase">250+</span>
            </span>
          </div>
          <!-- End Avatar Group -->
        </div>
        ...
        <div class="pt-5 sm:pt-0 sm:ps-5">
          <div class="text-lg font-semibold text-gray-800">Di percayai oleh</div>
          <div class="text-sm text-gray-500">250 Orang</div>
        </div>
      </div>
      <!-- End Avatar Group -->
...
<form id="contactForm">
  <div class="mx-auto max-w-2xl sm:flex sm:space-x-3 p-3 bg-white border rounded-lg shadow-lg shadow-gray-100">
    <div class="w-full pb-2 sm:pb-0">
      <label for="hs-hero-name-1" class="block text-sm font-medium"><span class="sr-only">Nama</span></label>
      <input type="text" id="hs-hero-name-1" class="py-3 px-4 block w-full border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Nama">
    </div>
    <div class="pt-2 sm:pt-0 sm:ps-3 border-t border-gray-200 sm:border-t-0 sm:border-s w-full">
      <label for="hs-hero-email-1" class="block text-sm font-medium"><span class="sr-only">Nama Sekolah/Institusi</span></label>
      <input type="text" id="hs-hero-email-1" class="py-3 px-4 block w-full border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Nama Sekolah/Institusi">
    </div>
    <div class="whitespace-nowrap pt-2 sm:pt-0 grid sm:block">
      <a id="whatsappLink" target="_blank" class="py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-green-700 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">
        Connect With Us!
      </a>
    </div>
  </div>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    const nameInput = document.getElementById("hs-hero-name-1");
    const schoolInput = document.getElementById("hs-hero-email-1");
    const whatsappLink = document.getElementById("whatsappLink");

    function updateWhatsAppLink() {
      const nama = encodeURIComponent(nameInput.value.trim());
      const namaSekolah = encodeURIComponent(schoolInput.value.trim());
      const baseUrl = "https://wa.me/6288294744259";
      const message = `Halo Kak, Saya ${nama} Asal Sekolah ${namaSekolah} Ingin Bertanya Tentang Pemuda Mendunia`;

      whatsappLink.href = `${baseUrl}?text=${message}`;
    }

    nameInput.addEventListener("input", updateWhatsAppLink);
    schoolInput.addEventListener("input", updateWhatsAppLink);
  });
</script>
